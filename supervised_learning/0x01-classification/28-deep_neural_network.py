...
class DeepNeuralNetwork:
    ...
    @property
    def activation(self):
        """ Getter method to private variable activation.
        """
    ...
    def evaluate(self, X, Y):
        ...
        J = self.cost(Y, A[0])
        A = np.where(A[0] == np.amax(A[0], axis=0), 1, 0)
        return A, J
    ...
    def gradient_descent(self, Y, cache, alpha=0.05):
        ...
        for layer in range(self.L, 0, -1):
            ...
            if self.activation is 'sig':
                ...
                dz = np.matmul(weights.T, dz) * (1 - A ** 2)
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True,
        ...
        for i in range(iterations):
            temp_J = self.evaluate(X, Y)
            self.gradient_descent(Y, self.cache, alpha)
            if i % step == 0 and verbose:
                print("Cost after {} iterations: {}".format(i, temp_J[1]))
                y_cost.append(temp_J[1])
                steps.append(i)
        ...
        if graph:
            ...
            plt.ylabel("cost")
            plt.plot(steps, y_cost, 'b')
            plt.show()

        final_J = self.evaluate(X, Y)
